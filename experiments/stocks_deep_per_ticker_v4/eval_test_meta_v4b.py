"""Evaluate META v4 top configs (trend template corregido) on train+test."""
...
# Best CR: atr=2.0, HL, mda=None, red=0.8, lb=63, tol=0.2, mdp=0.25, alt=0.03, comp=0.85
META_DET_BEST_CR = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.80, "lookback_bars": 63, "compression_threshold": 0.85,
    "tolerance": 0.20, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.03,
    "volume_contraction": None, "trend_template": True}

# Best WR: atr=2.0, HL, mda=None, red=0.4, lb=63, tol=0.1, mdp=0.25, alt=0.01, comp=0.85
META_DET_BEST_WR = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.40, "lookback_bars": 63, "compression_threshold": 0.85,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None, "trend_template": True}

CONFIGS = [
    {"name": "v4#1 best_CR no_filter tr=2.5 early=3d be=1.5 sl=3%",
     "det": {**META_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": None, "early_exit_days": 3,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},

    {"name": "v4#2 best_CR no_filter tr=2.5 tg=2R early=3d be=1.5 sl=3%",
     "det": {**META_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 2.0, "early_exit_days": 3,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},

    {"name": "v4#3 best_WR no_filter tr=2.5 be=1.5 sl=3%",
     "det": {**META_DET_BEST_WR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},

    {"name": "v4#4 best_CR no_filter tr=2.0 tg=5R be=1.0 sl=5% (agresivo)",
     "det": {**META_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.0, "target_r_multiple": 5.0, "early_exit_days": None,
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"  META — Evaluacion Train + Test (v4b TT solo primer pico, sin cond 2/3/4 formacion)")
    print(f"{'='*70}")
