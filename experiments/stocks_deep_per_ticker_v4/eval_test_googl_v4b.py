"""Evaluate GOOGL v4 top configs (trend template corregido) on train+test."""
...
# Best CR (unica det): atr=3.0, HL, mda=None, red=0.6, lb=126, tol=0.1, mdp=0.25, alt=0.01, comp=0.85
GOOGL_DET = {"atr_mult": 3.0, "use_close_only": False, "max_depth_atr": None,
...
CONFIGS = [
    {"name": "v4#1 no_filter tr=2.5 tg=5R be=1.0 sl=3%",
     "det": {**GOOGL_DET, "vol_filter": "no_filter"},
    ...
    {"name": "v4#2 no_filter tr=2.5 tg=3R be=0.5 sl=5% (best WR)",
     "det": {**GOOGL_DET, "vol_filter": "no_filter"},
    ...
    {"name": "v4#3 no_filter tr=2.5 be=1.0 sl=3% (sin target)",
     "det": {**GOOGL_DET, "vol_filter": "no_filter"},
...
if __name__ == "__main__":
    daily = pd.read_csv(DATA_DIR / "GOOGL.csv", parse_dates=["date"], index_col="date")
    ...
    print(f"{'='*70}")
    print(f"  GOOGL — Evaluacion Train + Test (v4b TT solo primer pico, sin cond 2/3/4 formacion)")
    print(f"{'='*70}")
