"""Evaluate AMZN v4 top configs (trend template corregido) on train+test."""
...
# Best CR: atr=3.0, C, mda=None, red=0.6, lb=126, tol=0.1, mdp=0.25, alt=0.03, comp=0.95, VC=False, TT=True
AMZN_DET_BEST_CR = {"atr_mult": 3.0, "use_close_only": True, "max_depth_atr": None,
...
# Alt det (Phase 2 #3): atr=4.0, HL, mda=None, red=0.6, lb=126, tol=0.1, mdp=0.25, alt=0.01, comp=0.85
AMZN_DET_ALT = {"atr_mult": 4.0, "use_close_only": False, "max_depth_atr": None,
...
# Best WR: atr=3.0, HL, mda=None, red=0.6, lb=126, tol=0.1, mdp=0.25, alt=0.01, comp=0.85
AMZN_DET_BEST_WR = {"atr_mult": 3.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.60, "lookback_bars": 126, "compression_threshold": 0.85,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None, "trend_template": True}

CONFIGS = [
    {"name": "v4#1 best_CR no_filter tr=2.0 tg=5R early=3d be=0.5 sl=5%",
     "det": {**AMZN_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.0, "target_r_multiple": 5.0, "early_exit_days": 3,
              "breakeven_r_multiple": 0.5, "max_stop_loss_pct": 0.05}},

    {"name": "v4#2 alt_det no_filter tr=2.0 tg=5R early=3d be=0.5 sl=5%",
     "det": {**AMZN_DET_ALT, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.0, "target_r_multiple": 5.0, "early_exit_days": 3,
              "breakeven_r_multiple": 0.5, "max_stop_loss_pct": 0.05}},

    {"name": "v4#3 best_WR w3_t1.2_f5 tr=3.0 tg=5R be=1.0 sl=5%",
     "det": {**AMZN_DET_BEST_WR,
             "vol_filter": {"window": 3, "threshold": 1.2, "forward": 5}},
     "exit": {"trailing_atr_multiplier": 3.0, "target_r_multiple": 5.0, "early_exit_days": None,
              "breakeven_r_multiple": 1.0, "max_stop_loss_pct": 0.05}},

    {"name": "v4#4 best_CR no_filter tr=2.5 be=1.0 sl=3% (conservador)",
     "det": {**AMZN_DET_BEST_CR, "vol_filter": "no_filter"},
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"  AMZN — Evaluacion Train + Test (v4b TT solo primer pico, sin cond 2/3/4 formacion)")
    print(f"{'='*70}")
