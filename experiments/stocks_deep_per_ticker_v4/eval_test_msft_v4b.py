"""Evaluate MSFT v4 top configs (trend template corregido) on train+test."""
...
# Best CR: atr=2.0, HL, mda=None, red=0.8, lb=126, tol=0.1, mdp=0.25, alt=0.03, comp=0.9, VC=True
MSFT_DET_BEST_CR = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.80, "lookback_bars": 126, "compression_threshold": 0.90,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.03,
    "volume_contraction": {"method": "ratio", "ratio_threshold": 0.85}, "trend_template": True}

# Best WR: atr=3.0, HL, mda=6, red=0.6, lb=126, tol=0.1, mdp=0.25, alt=0.01, comp=0.95, VC=False
MSFT_DET_BEST_WR = {"atr_mult": 3.0, "use_close_only": False, "max_depth_atr": 6,
...
CONFIGS = [
    {"name": "v4#1 best_CR no_filter tr=3.0 tg=2R early=3d be=1.0 sl=3%",
     "det": {**MSFT_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 3.0, "target_r_multiple": 2.0, "early_exit_days": 3,
              "breakeven_r_multiple": 1.0, "max_stop_loss_pct": 0.03}},

    {"name": "v4#2 best_WR no_filter tr=3.0 be=1.0 sl=5%",
     "det": {**MSFT_DET_BEST_WR, "vol_filter": "no_filter"},
    ...
    {"name": "v4#3 best_CR no_filter tr=2.5 be=1.0 sl=3% (sin target/early)",
     "det": {**MSFT_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.0, "max_stop_loss_pct": 0.03}},

    {"name": "v4#4 best_CR no_filter tr=3.0 tg=5R early=3d be=1.0 sl=5% (agresivo)",
     "det": {**MSFT_DET_BEST_CR, "vol_filter": "no_filter"},
...
if __name__ == "__main__":
    daily = pd.read_csv(DATA_DIR / "MSFT.csv", parse_dates=["date"], index_col="date")
    ...
    print(f"{'='*70}")
    print(f"  MSFT — Evaluacion Train + Test (v4b TT solo primer pico, sin cond 2/3/4 formacion)")
    print(f"{'='*70}")
