"""Evaluate NVDA v4 top configs (trend template corregido) on train+test."""
...
def apply_trend_template_filter(signals, tt_data):
    """Filter signals using correct Minervini Stage 2 (v4).
...
# Best CR (Phase 2 #1): atr=3.0, HL, mda=None, red=0.8, lb=126, tol=0.1, mdp=0.25, alt=0.03, comp=0.95, VC=False, TT=True
NVDA_DET_BEST_CR = {"atr_mult": 3.0, "use_close_only": False, "max_depth_atr": None,
...
# Best WR (Phase 2): atr=2.0, HL, mda=None, red=0.6, lb=63, tol=0.1, mdp=0.25, alt=0.01, comp=0.9, VC=False, TT=True
NVDA_DET_BEST_WR = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.60, "lookback_bars": 63, "compression_threshold": 0.90,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None, "trend_template": True}

CONFIGS = [
    {"name": "v4#1 best_CR no_filter tr=2.5 tg=5R early=5d be=1.0 sl=5%",
     "det": {**NVDA_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 5.0, "early_exit_days": 5,
              "breakeven_r_multiple": 1.0, "max_stop_loss_pct": 0.05}},

    {"name": "v4#2 best_CR no_filter tr=2.5 tg=5R early=5d be=1.5 sl=5%",
     "det": {**NVDA_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 5.0, "early_exit_days": 5,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.05}},

    {"name": "v4#3 best_CR no_filter tr=2.5 tg=5R early=5d be=2.0 sl=5%",
     "det": {**NVDA_DET_BEST_CR, "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 5.0, "early_exit_days": 5,
              "breakeven_r_multiple": 2.0, "max_stop_loss_pct": 0.05}},

    {"name": "v4#4 best_WR w1_t1.2_f3 tr=1.0 tg=3R be=0.5 sl=5%",
     "det": {**NVDA_DET_BEST_WR,
    ...
    {"name": "v4#5 best_CR no_filter tr=2.5 be=1.0 sl=3% (sin target/early)",
     "det": {**NVDA_DET_BEST_CR, "vol_filter": "no_filter"},
...
if __name__ == "__main__":
    daily = pd.read_csv(DATA_DIR / "NVDA.csv", parse_dates=["date"], index_col="date")
    ...
    print(f"{'='*70}")
    print(f"  NVDA — Evaluacion Train + Test (v4b TT solo primer pico, sin cond 2/3/4 formacion)")
    print(f"{'='*70}")
