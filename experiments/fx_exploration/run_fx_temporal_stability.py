"""Temporal stability experiment: find best VCP params on TRAIN (2015-2019),
including swing atr_mult, and test out-of-sample on TEST (2020-2026).

Supports any FX currency. By default runs all 5 pairs.
Results are tracked in MLflow with combined pattern+trade plots.

Usage:
    python run_fx_temporal_stability.py                # all currencies
    python run_fx_temporal_stability.py EURUSD GBPUSD  # specific currencies
"""
import sys
import tempfile
import time
...
print(f"Por moneda: {n_det} configs deteccion + {n_exit} configs salida")
...
def run_single_currency(ticker, daily, train, test):
    """Run full temporal stability analysis for one currency with MLflow tracking."""
    ...
    # Compute strategy + buy & hold metrics per split
    metrics_by_split = {}
    ...
    # Log to MLflow — one nested run per split with plots
    for split_name in ["TRAIN", "TEST", "FULL"]:
        n_sig, ev = ev_by_split[split_name]
        ohlc = splits[split_name]
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="temporal_stability") as parent_run:
        for ticker in TICKERS:
        ...
        # Log cross-currency summary as artifacts
    ...
    if len(all_results) > 1:
        print(f"\n\n{'#'*70}")
        print(f"  COMPARACION ENTRE MONEDAS — PARAMETROS OPTIMOS")
        print(f"{'#'*70}")
        ...
        # ── Comparative table: strategy vs buy & hold on TEST ──
