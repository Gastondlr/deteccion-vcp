"""Deep per-ticker optimization for AAPL.
...
New params vs previous experiments:
...
print(f"AAPL Deep Per-Ticker Optimization")
...
det_df.to_csv(output_dir / "aapl_phase1_detection.csv", index=False)
print(f"\n  Guardado: {output_dir / 'aapl_phase1_detection.csv'}")

if len(signals_train) > 0:
    exit_df.to_csv(output_dir / "aapl_phase2_exit.csv", index=False)
    print(f"  Guardado: {output_dir / 'aapl_phase2_exit.csv'}")
