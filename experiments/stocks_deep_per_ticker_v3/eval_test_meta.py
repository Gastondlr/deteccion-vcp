"""Evaluate META v2+v3 top configs on train+test."""
...
# Det A: v2 #1 — red=0.80, lb=126, VC=True (high trade count)
META_DET_A = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
...
# Det B: v2 #2/#4 — red=0.60, lb=126, VC=False (moderate)
META_DET_B = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.60, "lookback_bars": 126, "compression_threshold": 0.95,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None}

# Det C: v2 #2/v3 #1 — red=0.60, lb=63, VC=False (short lookback)
META_DET_C = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": 6.0,
    "min_total_reduction": 0.60, "lookback_bars": 63, "compression_threshold": 0.95,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None}

# Det D: v3 #3 — red=0.80, lb=63, tol=0.20, VC=False
META_DET_D = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.80, "lookback_bars": 63, "compression_threshold": 0.95,
    "tolerance": 0.20, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.03,
    "volume_contraction": None}

CONFIGS = [
    # v2 baseline: best composite, high trade count
    {"name": "v2#1 no_filter VC tr=1.5 tg=2R sl=3%",
     "det": {**META_DET_A, "vol_filter": "no_filter"},
    ...
    # v2: w1_t1.2 backward, no VC, lb=126
    {"name": "v2#4 w1_t1.2 tr=2.5 tg=2R sl=7%",
     "det": {**META_DET_B, "vol_filter": {"window": 1, "threshold": 1.2}},
    ...
    # v3: forward w1_t1.2_f3, lb=63
    {"name": "v3#1 w1_t1.2_f3 tr=2.5 tg=2R sl=7%",
     "det": {**META_DET_C, "vol_filter": {"window": 1, "threshold": 1.2, "forward": 3}},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 2.0, "early_exit_days": None,
              "breakeven_r_multiple": 0.5, "max_stop_loss_pct": 0.07}},

    # v3: forward w1_t1.2_f3, lb=126, VC=True
    {"name": "v3#4 w1_t1.2_f3 VC tr=2.5 tg=2R sl=7%",
     "det": {**META_DET_B, **{"volume_contraction": {"method": "ratio", "ratio_threshold": 0.85}},
             "vol_filter": {"window": 1, "threshold": 1.2, "forward": 3}},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 2.0, "early_exit_days": None,
              "breakeven_r_multiple": 0.5, "max_stop_loss_pct": 0.07}},

    # v3: forward w3_t1.5_f3, different det, target=5R
    {"name": "v3#3 w3_t1.5_f3 tr=3.0 tg=5R early=3 sl=5%",
     "det": {**META_DET_D, "vol_filter": {"window": 3, "threshold": 1.5, "forward": 3}},
     "exit": {"trailing_atr_multiplier": 3.0, "target_r_multiple": 5.0, "early_exit_days": 3,
              "breakeven_r_multiple": 1.0, "max_stop_loss_pct": 0.05}},
]

if __name__ == "__main__":
    daily = pd.read_csv(DATA_DIR / "META.csv", parse_dates=["date"], index_col="date")
    ...
    print(f"{'='*70}")
    print(f"  META — Evaluacion Train + Test (v2 + v3 forward volume)")
    print(f"{'='*70}")
