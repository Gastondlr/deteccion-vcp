"""Evaluate MSFT v3 top configs (forward volume) on train+test."""
...
# MSFT det params: atr=2.0, close=False, mda=nan, red=0.6, lb=126, comp=0.85
# Forward top uses VC=False, baseline top uses VC=True
MSFT_DET_BASE = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
...
CONFIGS = [
    {"name": "FWD#1 w3_t1.2_f3 tr=3.0 be=1.5 sl=3%",
     "det": {**MSFT_DET_BASE, "volume_contraction": None,
    ...
    {"name": "FWD#2 w1_t1.2_f3 tr=3.0 be=1.5 sl=3%",
     "det": {**MSFT_DET_BASE, "volume_contraction": None,
             "vol_filter": {"window": 1, "threshold": 1.2, "forward": 3}},
     "exit": {"trailing_atr_multiplier": 3.0, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},

    {"name": "Baseline no_filter tr=1.5 be=0.5 sl=3% VC=True",
     "det": {**MSFT_DET_BASE, "volume_contraction": {"method": "ratio", "ratio_threshold": 0.85},
             "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 1.5, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 0.5, "max_stop_loss_pct": 0.03}},

    {"name": "Baseline no_filter tr=3.0 be=1.5 sl=3% VC=False (same exit as fwd)",
     "det": {**MSFT_DET_BASE, "volume_contraction": None,
             "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 3.0, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"  MSFT — Evaluacion Train + Test (v3 forward volume)")
    print(f"{'='*70}")
