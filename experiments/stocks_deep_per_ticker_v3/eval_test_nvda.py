"""Evaluate NVDA v3 top configs (forward volume) on train+test."""
...
# NVDA forward top det: atr=2.0, close=False, mda=None, red=0.6, lb=63, tol=0.1, mdp=0.25, alt=0.01, comp=0.95, VC=False
NVDA_DET_FWD = {"atr_mult": 2.0, "use_close_only": False, "max_depth_atr": None,
...
# NVDA no_filter top det: atr=3.0, close=False, mda=None, red=0.8, lb=63, comp=0.85, VC=False
NVDA_DET_NF = {"atr_mult": 3.0, "use_close_only": False, "max_depth_atr": None,
    "min_total_reduction": 0.80, "lookback_bars": 63, "compression_threshold": 0.85,
    "tolerance": 0.10, "max_depth_pct": 0.25, "ascending_lows_tolerance": 0.01,
    "volume_contraction": None}

CONFIGS = [
    {"name": "FWD#1 w3_t1.2_f3 tr=2.5 be=1.5 sl=3%",
     "det": {**NVDA_DET_FWD,
    ...
    {"name": "FWD#2 w1_t1.2_f3 tr=2.5 be=1.5 sl=3%",
     "det": {**NVDA_DET_FWD,
             "vol_filter": {"window": 1, "threshold": 1.2, "forward": 3}},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},

    {"name": "FWD#3 w3_t1.2_f3 tr=2.5 tg=5R be=1.0 sl=5%",
     "det": {**NVDA_DET_FWD,
             "vol_filter": {"window": 3, "threshold": 1.2, "forward": 3}},
    ...
    {"name": "Baseline no_filter tr=2.5 tg=5R be=2.0 sl=3% (best NF det)",
     "det": {**NVDA_DET_NF,
             "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": 5.0, "early_exit_days": None,
              "breakeven_r_multiple": 2.0, "max_stop_loss_pct": 0.03}},

    {"name": "Baseline no_filter tr=2.5 be=1.5 sl=3% (same det as fwd)",
     "det": {**NVDA_DET_FWD,
             "vol_filter": "no_filter"},
     "exit": {"trailing_atr_multiplier": 2.5, "target_r_multiple": None, "early_exit_days": None,
              "breakeven_r_multiple": 1.5, "max_stop_loss_pct": 0.03}},
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"  NVDA — Evaluacion Train + Test (v3 forward volume)")
    print(f"{'='*70}")
