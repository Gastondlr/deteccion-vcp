"""Deep per-ticker VCP optimization v3 — con forward volume confirmation.

Mismo grid de deteccion que v2, pero expande las variantes de volumen para
incluir confirmacion forward (dias despues del breakout de precio).

Para variantes forward, si el volumen no confirma en la ventana backward,
se busca en los N dias siguientes verificando que el precio siga por encima
del pivot. Si confirma forward, la fecha de entrada y el precio se ajustan
al dia de confirmacion.

Usage:
    python3 run_deep_with_forward.py MSFT
    python3 run_deep_with_forward.py AAPL MSFT --top-n 10 --min-trades 3
"""
import argparse
import functools
import itertools
...
def run_ticker(ticker, train_cutoff, top_n, min_trades):
    """Run full v3 optimization for a single ticker (train only)."""

    print(f"\n{'#'*70}")
    print(f"  {ticker} — Deep Per-Ticker Optimization v3 (with forward volume)")
    print(f"{'#'*70}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deep per-ticker VCP optimization v3 (with forward volume)")
    ...
    print(f"Deep Per-Ticker VCP Optimization v3 (with forward volume)")
