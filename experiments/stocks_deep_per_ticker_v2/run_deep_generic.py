...
Improvements over v1:
  - Generic: parameterized by ticker via CLI
  - Volume breakout confirmation as systematic post-filter (7 variants)
  - 3 exit profiles in Phase 1 (trail 1.5/2.5/3.5) for robust ranking
  - Multi-criteria selection: top N by avg_CR + top N by avg_WR
  - Phase 2 runs on all candidates, not just top 1
  - Composite final ranking: 50% CR + 30% WR + 20% avg_R

Usage:
    python3 run_deep_generic.py AAPL
    python3 run_deep_generic.py AAPL AMZN GOOGL --top-n 10 --min-trades 3
"""
import argparse
import functools
import itertools
...
def apply_volume_post_filter(signals, ohlc, window, threshold, lookback_days=50):
    ...
    Mirrors _evaluate_volume logic in pivot_breakout.py:298-360.
    For each signal, checks if any day in [date - window + 1, date]
    has volume >= threshold * avg(volume over lookback period).
    """
    filtered = {}
...
def run_ticker(ticker, train_cutoff, top_n, min_trades):
    """Run full v2 optimization for a single ticker (train only)."""

    print(f"\n{'#'*70}")
    print(f"  {ticker} — Deep Per-Ticker Optimization v2")
    print(f"{'#'*70}")
                                    ...
                                    for comp_thresh in COMPRESSION_THRESHOLDS:
                                        ...
                                        for vol_contr in VOLUME_CONTRACTION_OPTIONS:
                                            ...
                                            for use_tt in TREND_TEMPLATE_OPTIONS:
                                                ...
                                                if use_tt:
                                                    tt_filtered = {dt: s for dt, s in raw_signals.items()
                                                ...
                                                for vf in VOLUME_FILTER_VARIANTS:
                                                    if vf["window"] is None:
                                                        ...
                                                        vol_filtered = apply_volume_post_filter(
                                                            tt_filtered, train, vf["window"], vf["threshold"])
    ...
    for dim_name, dim_values, col_name in sensitivity_dims:
        ...
        for val in dim_values:
            ...
            med_cr = has_trades["avg_CR"].median() if len(has_trades) > 0 else 0
    ...
    if len(has_t) > 0:
        ...
        print(f"\n  Promedio CR por perfil:")
        for prof in ["CR_tight", "CR_medium", "CR_loose"]:
            print(f"    {prof:>10s}: mean={has_t[prof].mean():>+.4f}, "
                  f"best={has_t[prof].max():>+.4f}")

    # ── Top 30 by avg_CR ────────────────────────────────────

    det_df_by_cr = det_df.sort_values("avg_CR", ascending=False)
    det_df_by_wr = det_df.sort_values(["avg_WR", "avg_CR"], ascending=[False, False])
    ...
    for cand_idx, (_, cand) in enumerate(candidates.iterrows()):
        ...
        print(f"  Candidato {cand_idx+1}/{len(candidates)}: "
              f"{len(filtered_sigs)} senales, evaluando {n_exit} exits...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deep per-ticker VCP optimization v2")
    parser.add_argument("tickers", nargs="+", help="Tickers to process (e.g. AAPL AMZN)")
    ...
    print(f"Deep Per-Ticker VCP Optimization v2")
