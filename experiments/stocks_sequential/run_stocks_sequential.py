"""Sequential temporal stability experiment for stocks.

Same methodology as the FX sequential experiment:
...
- volume_ratio_threshold as variable (None/disabled, 1.0, 1.5, 2.0)
- trend_template as variable (True/False)
- TRAIN (2015-2019) / TEST (2020-2026) split

Parameters adapted for stocks vs FX:
- Larger stop loss (7% vs 2%)
- Standard Minervini depth limits (35% max)
- Higher ATR multipliers (1.0-4.0 vs 0.75-2.5)

Usage:
    python run_stocks_sequential.py                # all 8 stocks
    python run_stocks_sequential.py AAPL NVDA      # specific stocks
"""
import sys
import tempfile
import time
...
VOLUME_THRESHOLDS = [None, 1.0, 1.5, 2.0]
...
n_det = len(ATR_MULTS) * len(DEPTH_ATRS) * len(REDUCTIONS) * len(LOOKBACK_BARS) * len(VOLUME_THRESHOLDS) * len(TREND_TEMPLATE_OPTIONS)
n_exit = len(TRAILING_MULTS) * len(TARGET_RS) * len(EARLY_EXITS) * len(BREAKEVEN_RS)
print(f"Stocks: {TICKERS}")
print(f"Por stock: {n_det} configs deteccion (lookback x vol x trend_template) + {n_exit} configs salida")
...
def make_breakout_params(vol_threshold):
    bp = {**BREAKOUT_BASE}
    if vol_threshold is None:
        bp["require_volume_confirmation"] = False
        bp["volume_ratio_threshold"] = 1.5
    else:
        bp["require_volume_confirmation"] = True
        bp["volume_ratio_threshold"] = vol_threshold
    return bp


def detect_signals(ohlc, seq_params, cache_entry, breakout_params, trend_template_mask=None):
    ...
    if trend_template_mask is not None:
        signals = {dt: s for dt, s in signals.items()
                   if dt in trend_template_mask.index and trend_template_mask.loc[dt]}
    return signals
...
def run_single_stock(ticker, daily, train, test):
    """Run full temporal stability analysis for one stock with sequential grouping."""
    ...
    print(f"  atr_mult x depth_atr x reduction x lookback x vol_thresh x trend_template")
    print(f"{'='*70}")
            ...
            for reduction in REDUCTIONS:
                for lookback in LOOKBACK_BARS:
                    ...
                    for vol_thresh in VOLUME_THRESHOLDS:
                        bp = make_breakout_params(vol_thresh)
                        for use_tt in TREND_TEMPLATE_OPTIONS:
                            mask = tt_mask_train if use_tt else None
                            signals = detect_signals(train, seq, cache, bp, mask)
                            ...
                            all_det_results.append({
                                ...
                                "vol_thresh": vol_thresh, "trend_template": use_tt,
    ...
    for _, row in det_df_sorted.head(20).iterrows():
        vt = row['vol_thresh'] if pd.notna(row['vol_thresh']) else 'None'
        tt = 'TT' if row['trend_template'] else 'noTT'
        print(f"  atr={row['atr_mult']:.1f}, d={int(row['depth_atr'])}, "
    ...
    print("\nImpacto trend_template:")
    for use_tt in TREND_TEMPLATE_OPTIONS:
        sub = det_df[det_df["trend_template"] == use_tt]
        has_trades = sub[sub["trades"] > 0]
        label = "TT=True " if use_tt else "TT=False"
        print(f"  {label}: avg_CR={sub['CR'].mean():+.2%}, best_CR={sub['CR'].max():+.2%}, "
              f"{len(has_trades)}/{len(sub)} con trades")

    print("\nImpacto volume_threshold:")
    for vt in VOLUME_THRESHOLDS:
        if vt is None:
            sub = det_df[det_df["vol_thresh"].isna()]
            label = "vol=None "
        else:
            sub = det_df[det_df["vol_thresh"] == vt]
            label = f"vol={vt:.1f} "
        has_trades = sub[sub["trades"] > 0]
        print(f"  {label}: avg_CR={sub['CR'].mean():+.2%}, best_CR={sub['CR'].max():+.2%}, "
              f"{len(has_trades)}/{len(sub)} con trades")
    ...
    best_vol_thresh = best_overall["vol_thresh"] if pd.notna(best_overall["vol_thresh"]) else None
    best_trend_template = bool(best_overall["trend_template"])
    print(f"\n>>> MEJOR CONFIG TRAIN: atr_mult={best_atr_mult}, "
          f"depth_atr={best_depth_atr}, reduction={best_reduction}, "
          f"lookback={best_lookback}, vol={best_vol_thresh}, TT={best_trend_template}")
    ...
    print(f"  atr_mult={best_atr_mult}, depth_atr={best_depth_atr}, "
          ...
          f"vol={best_vol_thresh}, TT={best_trend_template}")
    print(f"{'='*70}")

    best_seq = make_seq_params(best_depth_atr, best_reduction, best_lookback)
    best_bp = make_breakout_params(best_vol_thresh)
    best_cache = caches_train[best_atr_mult]
    best_tt_mask_train = tt_mask_train if best_trend_template else None
    signals_train = detect_signals(train, best_seq, best_cache, best_bp, best_tt_mask_train)
    ...
    print(f"    Filtros:   vol_thresh={best_vol_thresh}, trend_template={best_trend_template}")
    ...
    all_params = {
        ...
        "vol_thresh": str(best_vol_thresh),
        "trend_template": str(best_trend_template),
    ...
    for split_name, ohlc in splits.items():
        ...
        tt_mask = None
        if best_trend_template:
            tt_eval = evaluate_trend_template(ohlc)
            tt_mask = tt_eval["trend_template"]
        signals = detect_signals(ohlc, best_seq, cache, best_bp, tt_mask)
    ...
    return {
        ...
        "best_detection": {"depth_atr": best_depth_atr, "reduction": best_reduction, "lookback_bars": best_lookback,
                          "vol_thresh": best_vol_thresh, "trend_template": best_trend_template},
        "best_exit": best_exit,
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(run_name="sequential_stocks") as parent_run:
        for ticker in TICKERS:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for r in all_results:
                    ...
                    params_rows.append({
                        ...
                        "vol_thresh": d["vol_thresh"],
                        "trend_template": d["trend_template"],
    ...
    if len(all_results) > 1:
        print(f"\n\n{'#'*70}")
        print(f"  COMPARACION ENTRE STOCKS — PARAMETROS OPTIMOS (SEQUENTIAL)")
        print(f"{'#'*70}")
        ...
        for r in all_results:
            d = r["best_detection"]
            e = r["best_exit"]
            vt = str(d['vol_thresh']) if d['vol_thresh'] is not None else 'None'
            tt = 'Y' if d['trend_template'] else 'N'
            print(f"  {r['ticker']:<8s} | {r['best_atr_mult']:>5.1f} | {d['depth_atr']:>5d} | {d['reduction']:>5.2f} | "
